...
const SUGGESTIONS = [
  ...
  "Kategori apa yang paling banyak menguras uang?",
  "Apakah saya bisa nabung 5jt bulan depan?",
  "Tunjukkan tren pengeluaran 30 hari terakhir.",
];
...
export default function FinanceChatPage() {
  ...
  const { data: history, isLoading } = useQuery({
    queryKey: ["ai-finance-history"],
    queryFn: aiApi.financeQueryHistory,
  });

  const ask = useMutation({
    mutationFn: (q: string) => aiApi.financeQuery(q),
    onSuccess: (res, q) => {
      setMessages((prev) => [
        ...prev,
        ...
        {
          role: "assistant",
          text: res.answer ?? "Saya tidak menemukan jawaban.",
          ts: Date.now(),
        },
      ]);
      qc.invalidateQueries({ queryKey: ["ai-finance-history"] });
      setTimeout(() => {
  ...
  const clearHistory = useMutation({
    mutationFn: aiApi.deleteFinanceQueryHistory,
    onSuccess: () => {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Tanya keuangan kamu dalam bahasa natural — jawaban berdasarkan data
          asli.
        </p>
      </div>
                ...
                <div className="grid h-full place-items-center px-4 text-center">
                  <div>
                    ...
                    <p className="mt-3 font-semibold">
                      Mulai tanya tentang keuangan kamu
                    </p>
                    <p className="mt-1 text-sm text-muted-foreground">
                      Pilih saran di bawah atau ketik pertanyaan kamu.
                    </p>
